...
const controller = {
    ...
    if (req.caller.friends.length > 0) {
      const products = await Promise.all(
          req.caller.friends.map(async (friendId) => {
            ...
            const sharedProducts = friend.data().products.filter((product) => product.isShared && !product.isClaimed);
  ...
  claim: async (req, res) => {
    ...
    const claimee = await db.collection('users').doc(req.params.claimeeId).get();

    if (!claimer.friends.includes(claimee.id)) {
      res.status(400).json({ 'message': 'You can only claim products of friends.' });
    }

    const product = claimee.data().products.find(p => p.id === req.query.id && p.isShared);

    if (!product) {
      res.status(404).json({ 'message': 'Product was not found or was not shared.' });
    }
  ...
  acceptClaim: async (req, res) => {
    const claimer = await db.collection('users').doc(req.params.claimerId).get();
    const claimee = req.caller;

    const notification = claimee.notifications.find((n) => n.id === req.query.id);
    ...
    db.collection('users').doc(claimee.id).update({
      products: firestore.FieldValue.arrayRemove(product),
    }).then((result) => {
      product.isShared = false;
      product.isClaimed = true;

      db.collection('users').doc(claimer.id).update({
        products: firestore.FieldValue.arrayUnion(product),
      });
    }).catch((err) => {
      res.status(500).json({ 'message': 'Something went wrong while moving the product' });
    });

    await notificationsController.deleteNotification(claimee.id, notification.id);
