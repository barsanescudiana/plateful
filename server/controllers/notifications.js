...
async function sendPushIfEnabled(userId, text) {
  ...
  if (pushSubscription) {
    webpush
      .sendNotification(pushSubscription, JSON.stringify(push))
      .then((res) => {
        console.log(res);
...
const internalController = {
    ...
    switch (notification.type) {
      case "FRIEND_REQUEST": {
        ...
        await sendPushIfEnabled(
          requestee.id,
          `You got a friend request from ${notification.detail.from.firstName} ${notification.detail.from.lastName}!`
        );
      ...
      case "FRIEND_REQUEST_ACCEPTED": {
        ...
        await sendPushIfEnabled(
          to.id,
          `${notification.detail.from.firstName} ${notification.detail.from.lastName} is now your friend!`
        );
      ...
      case "CLAIM": {
        ...
        const result = await claimee.update({
          notifications: firestore.FieldValue.arrayUnion({
            ...
            detail: {
              fromUser: notification.detail.from.id,
              product: notification.detail.product,
            },
          }),
        });
        ...
        await sendPushIfEnabled(
          claimee.id,
          `${notification.detail.from.firstName} ${notification.detail.from.lastName} wants to claim one of your products!`
        );
      ...
      case "CLAIM_ACCEPTED": {
        ...
        await sendPushIfEnabled(
          to.id,
          `${notification.detail.from.firstName} ${notification.detail.from.lastName} accepted your claim!`
        );
  ...
  deleteNotification: async (userId, notificationUuid) => {
    const user = await db.collection("users").doc(userId).get();
    const notificationToDelete = user
      .data()
      .notifications.find((i) => i.id === notificationUuid);
...
const controller = {
    ...
    if (user.exists) {
      ...
      const notifications = await Promise.all(
        user.data().notifications.map(async (notification) => {
          const enrichedUser = await userController.getPublicInfoById(
            notification.detail.fromUser
          );
          if (notification.detail.product) {
            const enrichedProduct = await user
              .data()
              .products.find((p) => p.id === notification.detail.product);
  ...
  delete: async (req, res) => {
    internalController
      .deleteNotification(req.caller.id, req.params.notificationId)
      .then((result) => {
        if (result) {
